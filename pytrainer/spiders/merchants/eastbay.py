...
class Eastbay(MBase):
    ...
    def __init__(self, xreponse, url):
        ...
        self.url = url
        self.product_data = self.__getProductData()

    def extractProductData(self):
        item = PytrainerItem()

        try:
            item["category"] = "active skirts"
    ...
    def __getDescription(self):
        item_list = []
        node_collection = self.getElementValues("//h1[@id='product_title']|//div[@id='pdp_description']/p|//div["
                                                "@id='pdp_description']/ul/li")
        if node_collection is not None and len(node_collection) > 0:
            indx = 0
            for node in node_collection:
    ...
    def __getProductData(self):
        product_data = None
        product_data = self.getElementValues("//script[@id='productMainData']|//script[contains(text(), '\"id\":')]")
